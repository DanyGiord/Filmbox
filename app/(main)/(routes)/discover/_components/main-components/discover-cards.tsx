...
const DiscoverCards = () => {
  ...
  return (
    <div className="grid md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-7 sm:grid-cols-2 gap-4 w-full mb-7">
      {cards.map((card) => (
        <>
          {/* {skeleton ? (
          ...
          {card.poster_path && (
            ...
            >
              {skeleton && (
                <div className="animate-pulse w-full h-full rounded-2xl absolute inset-0 z-[9999]"></div>
              )}
              <div
                // @ts-ignore
              ...
              >
                ...
                <div
                  className={cn(
                    `absolute inset-0 p-3 w-full h-full flex flex-col justify-between rounded-2xl card_main group hover:backdrop-blur-[1px] hover:bg-black_second/50 overflow-hidden transition-all`
                  )}
                >
                  <div className="flex justify-between">
                    ...
                    >
                      {/* @ts-ignore */}
                      <button
                        onClick={() => addRemoveFavorites(card.id)}
                      ...
                      >
                        <Heart
                          className={cn(
                            ...
                            // @ts-ignore
                            searchFor === "movie"
                              ? favMovieIds.includes(card.id) &&
                                  "text-red fill-red"
                              : favSerieIds.includes(card.id) &&
                                  "text-red fill-red"
                          )}
                        />
                      </button>
                  ...
                  <div className="flex flex-col gap-y-2">
                    <div className="flex justify-start gap-x-2 translate-y-12 group-hover:-translate-y-2 duration-300">
                    ...
                    {/* max 2 linije texta */}
                    <p className="text-white_text text-center font-normal text-base hover:cursor-default grid place-items-center translate-y-12 group-hover:-translate-y-2 duration-300">
                      {/* @ts-ignore */}
