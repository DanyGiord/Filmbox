      ...
      const promise = removeFavMovie({ id: convexUserId, movieId })
        .then(() => {
          toast.success(
            <p>
              <b className="font-black">{title}</b> <br />{" "}
              <span className="text-[#FF4B4B]">removed</span> from Favorite
              Movies
            </p>,
            {
              style: {
        ...
        .catch((error) => {
          ...
          toast.error(
            <p>
              An error occured while{" "}
              <span className="text-[#FF4B4B]">removing</span> <br />{" "}
              <b className="font-black">{title}</b> <br /> to Favorite Movies
            </p>,
            {
              style: {
      ...
      const promise = addFavSerie({ id: convexUserId, serieId })
        .then(() => {
          toast.success(
            <p>
              <b className="font-black">{title}</b> <br />{" "}
              <span className="text-[#61D345]">added</span> to Favorite Series
            </p>,
            {
              style: {
                background: "#1a1a1a",
        ...
        .catch((error) => {
          ...
          toast.error(
            <p>
              An error occured while{" "}
              <span className="text-[#61D345]">adding</span> <br />{" "}
              <b className="font-black">{title}</b> <br /> to Favorite Series
            </p>,
            {
              style: {
                background: "#1a1a1a",
      ...
      const promise = addFavActor({ id: convexUserId, actorId })
        .then(() => {
          toast.success(
            <p>
              <b className="font-black">{title}</b> <br />{" "}
              <span className="text-[#61D345]">added</span> to Favorite Actors
            </p>,
            {
              style: {
                background: "#1a1a1a",
        ...
        .catch((error) => {
          ...
          toast.error(
            <p>
              An error occured while{" "}
              <span className="text-[#61D345]">adding</span> <br />{" "}
              <b className="font-black">{title}</b> <br /> to Favorite Actors
            </p>,
            {
              style: {
                background: "#1a1a1a",
            ...
            >
              ...
              >
                    ...
                    >
                      <Heart
                        className={cn(
                          ...
                          // @ts-ignore
                          searchFor === "movie" &&
                            favMovieIds?.includes(id) &&
                            "text-red fill-red",
                          searchFor === "tv" &&
                            favSerieIds?.includes(id) &&
                            "text-red fill-red",
                          searchFor === "actor" &&
                            favActorIds?.includes(id) &&
                            "text-red fill-red"
                        )}
                      />
                    </button>
                ...
                <div className="flex flex-col gap-y-2">
                        ...
                        <Badge className="flex justify-center items-center gap-x-1 text-sm text-gray">
                          ...
                          <div className="relative">
                            ...
                            <div
                              ...
                              style={{
                                width: `${(vote_average / 10) * 100 - 5}%`,
                              }}
                            >
                              <Star className="w-4 h-4 fill-[#ffa800] text-transparent" />{" "}
                              {/* Popunjena zvezda */}
                            </div>
                          </div>
                          {/* @ts-ignore */}
                  ...
                  <p
                    className={cn(
                      "text-white_text min-h-[48px] text-center font-normal text-base hover:cursor-default grid place-items-center transition-all translate-y-12 group-hover:-translate-y-2 duration-300",
                      searchFor === "actor" &&
                  ...
                  >
                    {/* @ts-ignore */}
                    {title &&
                      (fullTitle === id ? title : title.substring(0, 20))}
                    {fullTitle === id ? "" : title.length < 20 ? "" : "..."}
                  </p>
                  {searchFor !== "actor" && (
                    <>
                      {/* @ts-ignore */}
